...
class ClaudeClient:
    def __init__(self, settings: dict[str, Any]) -> None:
        ai = settings.get("ai", {})
        self.api_key = str(ai.get("claude_api_key", "")).strip()
        self.model = str(ai.get("claude_model", "claude-opus-4-5")).strip() or "claude-opus-4-5"
    ...
    def generate_meeting_notes(self, transcript: str, profile_context: str = "") -> NotesResult:
        if not transcript.strip():
            return NotesResult(text="", success=False, warning="No transcript text is available for Claude summarization.")
        if not self.api_key:
            return NotesResult(text="", success=False, warning="Claude API key is not configured.")
        ...
        try:
            ...
            if not text:
                return NotesResult(text="", success=False, warning="Claude returned an empty meeting note.")
            if not OllamaClient._has_required_sections(text, required_sections):
                ...
                warning_parts = ["Claude returned notes without the required Nova sections.", warning or "", retry_warning or ""]
                warning = " ".join(p for p in warning_parts if p).strip()
            return NotesResult(text=text, success=True, warning=warning)
        except Exception as error:
            return NotesResult(text="", success=False, warning=f"Claude summarization failed: {error}")
    ...
    def _call_api(self, prompt: str) -> tuple[str, str | None]:
        ...
        if not response.ok:
            return "", f"Claude API error: {response.status_code} {response.text[:500]}"
        payload = response.json()
        content = payload.get("content", [])
        text = str(content[0].get("text", "")).strip() if content else ""
